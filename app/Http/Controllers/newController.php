        ...
        if (!in_array($value ,$ff)){
            ...
            foreach ($ff as $key) {
              ...
              foreach ($doc  as $key1 ) 
                   { 
                     foreach ($key1 as $v => $value) { 
                     $TT=DB::table('index_terms')->join('words','words.id','=','index_terms.word_id')
                          ->where([['words.word','=',$v],['index_terms.file_id','=',$key]])->pluck('word_id')->toArray();
                        }
                        foreach ($TT as $key2 ) {
                          {
                              array_push($cosa, $key2);
                          }}}
                        $cos=0;
                         $arrres=array();  
                       foreach ($cosa as $key6 ) {
                        $ee=DB::table ('words')->where ('words.id','=',$key6)->pluck('idf')->toArray();
                        $eee=DB::table ('index_terms')->where ([['index_terms.word_id','=',$key6]
                          ,['index_terms.file_id','=',$key]])->pluck('tf_idf')->toArray();
                       ...
                       if(self::normalise_file($key)){

                       $cos=$cos/(self::normalise_file($key)*self::normalise_query($r));
                       $end[$key]=$cos;
                    
                     $ee=DB::table ('fileees')->where ('fileees.id','=',$key)->pluck('title')->toArray();
                                    }}
   
return view("result",compact('end','ee'));
}

public function ss($file_id)
{
  $title = DB::table ('fileees')->where ('fileees.id','=',$file_id)->pluck('title')->toArray();

  foreach ($title as $key => $value) {

    $contents = File::get($value); 
  $ss  = substr($value, 15);
  $ss1 =  substr($ss, 0, -4);

  } 

  return view("ss",compact('title','contents','ss1'));
}
                ...
                echo "faaaaaaat5";
                 ...
                 // var_dump($TT);
                    ...
                    echo "faaaaaaat8";
        		       	...
        		       	foreach ($ee as $e1) {echo "faaaaaaat9";
        		       		foreach ($eee as $e2) {
                        echo "faaaaaaat10";
        		       			//echo "<br>e1";
        		       			//echo $e1;
                   ...
                   if(self::normalise_file($key)){
        		       ...
        		       $cos=$cos/(self::normalise_file($key)*self::normalise_query($query));
        		       //array_push($end, $cos);
          		       echo"<br>";
          		      // echo $key;
                   $end[$key]=$cos;
                  //var_dump( $ee=DB::table ('fileees')->where ('fileees.id','=',$key)->pluck('title')->toArray());
...
public function insert_all($s="../storage/app/*.txt")
{
 ...
 //---------- Insert Word And File And Index Term To DB ----------
   public function insert_word($file_name)
{
   
    $file1 = new fileee;
    $file1->title = $file_name;
    $file1->num_of_word= self::word_all_file($file_name); //count of words in File
    $file1->save();
...
public function word_all_file($file_name="../storage/d3.txt")
{
...
//---------- Read Content File ----------
public function read_file($file_name="../storage/d3.txt")
{try
  {
...
{ 
  $tokens= self::stop_w($file_name)["1"];//Remove stop word
  print_r($tokens);
  $token_count = 0;
  $irregular_words = self::irr()["1"];
  //var_dump($irregular_words);
  $array = array();
        foreach($tokens as $token)
        { 
            $token_count++;
            ...
            $token = self::Stem($token);  //Stemming By using porter2 Algorithm
            $token = self::lemma($token); //lemmetization using  Skyeng\Lemmatizer library
...
//----------  indexing Calculate word  frquenc in file by n_gram algorithm ----------
public function countt($file_name="../storage/d3.txt")
{
